...
# imagemagick
magick_cmd = '/opt/homebrew/bin/magick'
...
def cropcorner(pdfin) -> subprocess.CompletedProcess[bytes]:
    ...
    x = subprocess.run(
        [gscmd, '-q', '-o', 'crop_' + pdfin, '-sDEVICE=pdfwrite',
         f'-g{sizestr}', '-c',
         f'"<</PageOffset [{offsetstr}]>> setpagedevice"', '-f', pdfin])

    return x


def ocrcorner(pdfin) -> subprocess.CompletedProcess[bytes]:
    """run ocr on the cropped corner"""
    logging.info(f'\t...running OCR on crop_{pdfin}')
    x = subprocess.run([gscmd, '-q', '-o', 'crop_' + pdfin + '-%03d.txt',
                        '--permit-file-read=$TESSDATA_PREFIX', '-sDEVICE=ocr',
                        '-r600', '-dDownScaleFactor=3', 'crop_' + pdfin],
                       env=my_env)
    return x
...
def get_assessment_from_user(page, expected_assessment=None) -> str:
    logger.warning(f'Trouble on page {page}. Expected {expected_assessment}.')
...
def process_assessment(pdfin, assessment_dict) -> subprocess.CompletedProcess[bytes]:
    ...
    for key in assessment_dict:
        ...
        logging.debug(' '.join(
            [gscmd, '-q', '-o', assName, '-sPageList=' + pagelist,
             '-sDEVICE=pdfwrite', pdfin]))
        x = subprocess.run(
            [gscmd, '-q', '-o', assName, '-sPageList=' + pagelist,
             '-sDEVICE=pdfwrite', pdfin])
...
def add_annotations_to_pdf(pdfin, file_name_in, assessment_name,
                           original_page_list) -> subprocess.CompletedProcess[bytes]:
...
def main() -> None:
    ...
    for item in sys.argv[2:]:
        ...
        process_assessment(item, assessmentdict)
        cleanup(item)
        logging.info('\t...done')
